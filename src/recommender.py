...
def get_similar_players(matrix, target_player_name, top_n=5):
    ...
    if target_player_name not in matrix.index:
        raise ValueError(f"Player '{target_player_name}' not in the filtered data.")
    ...
    normalized_df = pd.DataFrame(
        scaled_data, columns=matrix.columns, index=matrix.index
    )
...
if __name__ == "__main__":
    # Example usage (edit striker name to test)
    df = pd.read_parquet("data/WSL_2023-2024.parquet")
    df = filter_data(df)
    print(df)
    # data=get_wsl_data()
